...
class AuthController extends Controller
{
    ...
    public function registerGuest(Request $request)
    {
        ...
        // Generar QR de un solo uso con vigencia de 60 minutos
        $qrToken = \Illuminate\Support\Str::random(32);
        $user->update([
            'guest_qr_token' => $qrToken,
            'guest_qr_expires_at' => Carbon::now('UTC')->addMinutes(60),
            'guest_qr_used' => false,
        ]);
        ...
        return response()->json([
            ...
            'user' => $user,
            'qr_token' => $qrToken,
            'qr_content' => 'SENA-ACCESS:' . $user->user_identification . ':' . $qrToken,
            'qr_expires_at' => $user->guest_qr_expires_at,
        ], 201);
    }

    // Valida el QR de un invitado presentado en recepción (un solo uso, vigencia 60 min)
    public function validateGuestQr(Request $request)
    {
        ...
        if (!$user) {
            return response()->json(['message' => 'QR inválido: no corresponde a ningún invitado registrado.'], 400);
        }

        if ($user->guest_qr_used) {
            return response()->json(['message' => 'QR ya utilizado. Solicite uno nuevo en recepción.'], 400);
        }

        if ($user->guest_qr_expires_at && Carbon::parse($user->guest_qr_expires_at)->lt(Carbon::now('UTC'))) {
            return response()->json(['message' => 'QR expirado. Solicite uno nuevo en recepción.'], 400);
        }

        // Marcar como usado (un solo uso)
        $user->update(['guest_qr_used' => true]);

        // Registrar ingreso por validación de QR
        Ingreso::create([
            'ingreso_datetime' => Carbon::now('America/Bogota'),
            'ingreso_place' => 'Validación QR',
            'ingreso_type' => 'Entrada',
            'fk_id_user' => $user->id_usuario,
        ]);

        return response()->json([
            'message' => 'Acceso autorizado. QR validado correctamente.',
            'user' => [
